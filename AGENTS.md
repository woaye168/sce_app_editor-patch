...
**同时是 AGENT 操作编辑器的唯一 MCP 入口**：`sce_app_editor-patch mcp`（stdio，恒定 15 工具：editor_start/editor_stop/get_game_logs/capture_game/run_scenario 本地实现 + start_debug/stop_debug/publish_project/get_status 在线透传编辑器内 bgd_mcp_bridge + search_capabilities/describe_capability/invoke_capability/list_namespaces/get_events/set_suppress 在线透传桥 Gateway 元工具——编辑器完整能力目录经 search→invoke 触达，stdio 客户端无需直连桥 HTTP 端点）。**0.8.0 MCP 护栏**：tool_result 统一 32KB 截断+重试引导；get_game_logs 支持 match 正则（同条收纳×N 计数：剥离行首[时间][pid][级别][序号]比对）+ errors 段（错误行自动上浮，防前置错误致假阳性/假阴性漏判）；capture_game 支持 crop（游戏逻辑坐标，与 find_ui 同系）+ max_width（默认 1280）；get_events 本地切片（默认最新 50 条）；成功响应附一行 hint 引导下一步。**UI 闭环调试**（经 invoke_capability）：lua.find_ui 定位 → **lua.tap/lua.pick 复合一步**（0.8.2）→ capture_game(crop) 验证；0.8.2 起游戏侧全部走 **state 注入 + cgui 虚拟指针**（dbg_register 已删、控件零接入）：lua.hover_ui（真实悬停保持态）/drag_ui（拖放排序/偏移，on_drop 携载荷）/scroll_ui（pscroll 直驱）/key_down/key_up（键盘按住）/eval（游戏 VM 逃生舱，danger 级）/set_value（vp 点击轨道，返回实际生效值）/press_ui/release_ui/long_press_ui（均走 vp）——游戏侧经 lobby 跨 VM 总线直达 StateGame 的框架 dbg_bus 端点。**run_scenario 0.8.2 进化**：步骤变量（save_as + {$名} 引用）+ wait_for 轮询 + assert_text 断言。**0.8.7 多人调试适配**（设计哲学：不加工具不加能力面，只多可选玩家参数）：`start_debug` 加 `players`（人数 2~4 或逐玩家 [{player?,delay?}] 数组；缺省/=1 走单人现状一字不动；多人路径 = 桥 mp_debug.lua 自持互斥先停→自动选人→预校验→直调 debug_save_as→逐槽位轮询，返回 clients 归属清单）；`capture_game`/`lua.*`/`get_game_logs`/`run_scenario` 加 `player`（缺省=多人局 1 号/单人唯一；单人局带 player 自动回退+告知）；`get_status` 多返回 `clients` 数组（player/slot_id/user_id/online/paused）；`lua.set_pause{player,paused}` 同族新增（官方 tab 暂停按钮同款 disconnect/reconnect，断线/重连测试唯一手段，单人局报错无此能力）；dbg 寻址协议 v2（cmd 带 target/回执带 from+proto=2，单人局字节级兼容，旧框架多人定向响亮报错引导升级）；分玩家日志 = 桥常驻 tee（debug_client_info 投递→每玩家环形缓冲，logs.rs 复用 match/errors/同条收纳）；截图 player 定向内化切焦（mp_switch→等帧 200ms→WGC→restore 切回）。桥内部 /rpc 通道：mp_start/mp_switch/mp_logs（exe 直调，不进能力目录）。机制底档 doc/research/multi-player-debug.md。**0.8.10 轨道化**（设计哲学：纪律写进工具返回与结构，不靠提示词教育；需求与验收见 doc/requirements/0.8.10.md）：lua.eval description 双向界定（四合法用途/禁绕 UI）+ eval 调用反问 hint；run_scenario 定位修正（定稿归档用 scenario、探索期用短命令）+ 取证 op `verify_ui`（存在性断言+局部截图一步）/`check_logs`（errors 必零断言）+ 场景结束自动附 `log_summary`（期间新增行数/errors/warnings/top 重复行——刷屏类问题天然暴露）；`get_status`/run_scenario 附 `evidence` 取证计数（captures/ui_ops/eval_ops，eval 失衡+零截图附 hint）；UI 操作族 args.snapshot=true 视觉回执（操作后自动截控件区域小图）；get_game_logs 加 warns 段上浮（[warning]/[warn]，tee 通道「警告」同构）+ match ignore_case 参数；capture 系描述/hint 视觉通道指引（泛化不绑定具体视觉工具）；tap/pick 支持 scope/tag（与 find_ui 同语义）；invoke_capability 顶层 player 兼容并入 args+note 指正；scenario 步骤错误一律落 failed_step（op 分发闭包化，单步错误不再崩出整个场景）。**0.8.10 R6 服务端状态通道**：新增 lua.server_eval（danger 级同 eval）——客户端 dbg_bus 纯转发（`__async` 异步回执形态）→ 服务端 `libs/server/api/dbg_server.lua` handler（硬门 `base.test ~= nil` 仅测试环境注册，正式环境物理无入口），读服务端权威数据/调物品；require 翻译由构建盖戳 `_G.bgd_path_rules.modules_server` 驱动（配套 bgd_sce_tools：path_rules 盖戳位于 src/common/ 双端共享），盖戳缺失响亮报错提示重新构建。同名 CLI 子命令 `editor start|stop`、`logs [源] [行数] [match正则]`、`capture`、`notify <key>=<value>`（宿主解耦通知：切项目时更新运行时共享常量 bgd_runtime.lua + 最近项目 + 通知 GUI 刷新）供人类/脚本直接用。应用单实例；`--background` 静默驻留（看守线程 Win32 SW_HIDE/SW_RESTORE 驱动主窗口——egui 隐藏时事件循环休眠，不能依赖 ViewportCommand），驻留模式窗口 X = 隐藏（假关闭，服务常开，再次打开经 show 信号唤出），真退出只走宿主 `--quit`；前台启动窗口 X = 正常退出。bridge dll 部署失败（编辑器占用）时置待重部署标志，update 每 5s 自动重试。
...
- **Gateway 架构**：tools/list 恒定 10 个元工具，全部能力进能力目录（catalog.json 生成 + annotations.json 人工标注，均嵌入 dll）。通道：svc/datacore/cpp/cmd+lua/sys。安全分级 read/write/danger（**0.8.0 起 danger 默认放行**——AI 排障刚需；config.json `danger_deny` 可显式拒绝；write/danger 写审计日志）。Lua 桥支持延迟 ack。**0.8.0 能力目录瘦身**：svc.FileSystem 整体剔除（AI 用自身文件工具即可，减少搜索噪音）；新增 lua.find_ui/click_ui/click_at/input_text/press_ui/release_ui/long_press_ui/hover_ui/game_info（UI 闭环调试，游戏侧走 lobby 总线 → 框架 dbg_bus，机制见 doc/research/ui-reflection.md + lua-vm-bus.md）。**0.8.2 虚拟指针**：新增 lua.tap/pick/hover_ui（真实化）/drag_ui/scroll_ui/key_down/key_up/eval（游戏 VM 逃生舱 danger 级），游戏侧 dbg_register 删除、全部走 state 注入（机制见 doc/research/virtual-pointer.md）。
