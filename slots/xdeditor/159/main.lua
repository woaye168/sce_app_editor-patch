...
local debugMgr = SCE.GetDebugManager()
local UI_AUTHORING_VIEWPORT_ID = 'GamePlayInEditor'
...
local function show_editor_main_ui()
    ...
    _G.XDEDITOR_MAIN_UI_READY = true
...
if event_mgr then
    ...
    local function try_dispatch_ui_authoring_shortcut(event_name, event_data)
        if not debugMgr or not sceneMgr then
            return false
        end

        local ok_active, active = pcall(function()
            return debugMgr:is_ui_authoring_session_active(UI_AUTHORING_VIEWPORT_ID)
        end)
        ...
        local ok_focus, focused = pcall(function()
            return sceneMgr:is_game_in_editor_focus(UI_AUTHORING_VIEWPORT_ID)
        end)
        if not ok_focus or not focused then
            return false
        end

        event_data = event_data or {}
        event_data.viewport_id = UI_AUTHORING_VIEWPORT_ID
        GetMainFrame():SendEvent(event_name, event_data)
        return true
    end

    shortcutMgr.register(shortcutMgr.UNDO, function()
        if try_dispatch_ui_authoring_shortcut('UIAuthoringShortcutUndoRedo', { command = 'undo' }) then
            return
        end
        undo_redo_mgr:undo()
    end)
    shortcutMgr.register(shortcutMgr.REDO, function()
        if try_dispatch_ui_authoring_shortcut('UIAuthoringShortcutUndoRedo', { command = 'redo' }) then
            return
        end
        undo_redo_mgr:redo()
    end)
    ...
    shortcutMgr.register(shortcutMgr.SAVE, function()
        if try_dispatch_ui_authoring_shortcut('UIAuthoringShortcutSave') then
            return
        end
