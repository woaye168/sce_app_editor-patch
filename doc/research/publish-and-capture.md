...
## R2：调试态游戏画面截取（终版见 0.5.4：get_screen_rect + WGC 截窗 + 离屏恢复）

> 2026-08-19 修订：本节下述「截显示器」方案已被 0.5.4 取代（显示器截取受遮挡影响）。
> **终版见 doc/requirements/0.5.4.md**：WGC 直接截 WinUI 主窗口（重定向表面，遮挡无碍），
> 窗口最小化/隐藏时离屏恢复（SHOWNOACTIVATE + 屏外坐标 + WINDOWPLACEMENT 还原），
> 真后台成立；裁剪映射 = 逻辑坐标等比 + 内容区底对齐（帧像素坐标系闭环，无估算）。
> 另实测：SDL 内容窗口直接呈现，WGC/PrintWindow 均黑图，不可作为截取目标。
