...
pub fn set_module(
    ...
    if enable {
        ...
        if let Some(root) = inject_root {
            write_runtime_config(&patch_dir(lib_require_root), root)?;
        }
...
/// 运行时共享常量文件名（补丁框架根目录，所有模块可 require；表结构，未来加键不改结构）
pub const RUNTIME_CONFIG_FILE: &str = "bgd_runtime.lua";

/// 写运行时共享常量（`<require根>/sce_app_editor-patch/bgd_runtime.lua`）。
/// 内容 = `return { project_path = ... }` 表（宿主 notify/应用 refresh 时更新）
pub fn write_runtime_config(patch_dir_root: &Path, project_root: &Path) -> Result<(), String> {
    let normalized = project_root.to_string_lossy().replace('\\', "/");
    if normalized.contains("]]") {
        return Err("项目路径含非法字符 ]] ".into());
    }
    let content = format!(
        "-- AUTO-GENERATED by sce_app_editor-patch（运行时共享常量，宿主切换项目时自动更新），请勿手改\nreturn {{\n  project_path = [[{normalized}]],\n}}\n"
    );
    crypto::write_atomic(&patch_dir_root.join(RUNTIME_CONFIG_FILE), content.as_bytes())
}

/// 同步运行时共享常量（比对重写；顺带清理旧的 unwatch/_project_root.lua 注入残留）
pub fn sync_runtime_config(lib_require_root: &Path, project_root: &Path) -> Result<bool, String> {
    let root_dir = patch_dir(lib_require_root);
    let file = root_dir.join(RUNTIME_CONFIG_FILE);
    let normalized = project_root.to_string_lossy().replace('\\', "/");
    let existing = fs::read_to_string(&file).unwrap_or_default();
    if existing.contains(&format!("project_path = [[{normalized}]]")) {
        return Ok(false);
    }
    write_runtime_config(&root_dir, project_root)?;
    let stale = root_dir.join("unwatch").join("_project_root.lua");
    if stale.is_file() {
