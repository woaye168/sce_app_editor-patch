...
**同时是 AGENT 操作编辑器的唯一 MCP 入口**：`sce_app_editor-patch mcp`（stdio，恒定 8 工具：editor_start/editor_stop/get_game_logs/capture_game 本地实现 + start_debug/stop_debug/publish_project/get_status 在线透传编辑器内 bgd_mcp_bridge）。同名 CLI 子命令 `editor start|stop`、`logs`、`capture` 供人类/脚本直接用。应用单实例（GUI 重复启动只唤起窗口；`--background` 静默驻留；`--quit` 优雅退出）。
