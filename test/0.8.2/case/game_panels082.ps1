...
$scenario = @{
    ...
    steps = @(
        ...
        @{ op = 'invoke'; id = 'lua.click_ui'; args = @{ id = 'titlebar/close'; expect_absent = '整理背包' } },
        @{ op = 'wait_for'; q = '整理背包'; present = $false; timeout_ms = 4000 },

        @{ op = 'note'; text = 'GM 面（0.8.3 起 cgui 全功能可操作）：开发面板→填表单→关闭' },
        @{ op = 'invoke'; id = 'lua.tap'; args = @{ q = 'gm_box/gm'; expect = 'GM 面板' } },
        ...
        @{ op = 'invoke'; id = 'lua.click_ui'; args = @{ id = 'topbar/close' } },
