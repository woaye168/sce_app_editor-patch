...
**同时是 AGENT 操作编辑器的唯一 MCP 入口**：`sce_app_editor-patch mcp`（stdio，恒定 8 工具：editor_start/editor_stop/get_game_logs/capture_game 本地实现 + start_debug/stop_debug/publish_project/get_status 在线透传编辑器内 bgd_mcp_bridge）。同名 CLI 子命令 `editor start|stop`、`logs`、`capture` 供人类/脚本直接用。应用单实例 + **QQ/微信式托盘驻留**：关闭窗口=最小化到托盘（tray-icon 托盘菜单退出）；`--background` 静默驻留；唤起/退出由看守线程直接 Win32 驱动（PID 匹配找主窗口 ShowWindow；egui 隐藏时事件循环休眠，不能依赖 ViewportCommand）；`--quit` 优雅退出（QUIT 标志→主线程 Drop 托盘后关闭，防幽灵图标）。
